...
def find_similar_songs(file_name):
    ...
    data = pd.read_csv('./select_ft_csv/after_select_ft_final30s.csv',index_col=False)
    data = data.dropna()
    ...
    # 讀預測歌曲特徵
    userdf_3s2 = pd.read_csv(f'./musicfile/csv/30s/{file_name}/{file_name}.csv', index_col=False)
    ...
    userdf_3s2 .set_index(['songid'], inplace=True) # 設定 "songid" 為 index


    data = pd.concat([data, userdf_3s2], axis=0)
    # 將兩者欄位整合，會去掉videoname, url
    data = data.dropna(axis=1)

    # 將相同 “風格+編號”群組，並將所有特徵取平均值
    data = data.groupby(as_index=True, level=0).mean() # 以index 分群
    # 將index內的user欄位更名為file_name
    data = data.rename(index={'user':file_name})
    # data.set_index(['songid'], inplace=True) # 設定 "song_name" 為 index
    ...
    similar_top5 = tmpdf.join(songlist, on='songid', how='inner')
