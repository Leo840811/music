...
# file_name = 'crimeandlove'

def pca(file_name):


    parentpath = './pca_jpg/'
    pca_jpg_output = f'./pca_jpg/{file_name}/'
    if not os.path.isdir(parentpath):
        os.mkdir(parentpath)
    if not os.path.isdir(pca_jpg_output):
        os.mkdir(pca_jpg_output)

    data = pd.read_csv('./select_ft_csv/after_select_ft_final30s.csv',index_col=0)
    data = data.dropna()
    ...
    # 將user_music移至第一列，pca標籤顯示才會在第一個
    data_poke = data[data['label'] == file_name]
    data = data.drop(data_poke.index)
